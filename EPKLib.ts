namespace EPKLib {
  enum PassedResourceType {
    URL = "URL",
    DATA = "DATA",
  }

  export type LEPKDirectoryFileRaw = {
    filename: string;
    segments: string[];
    hash: string;
  };

  export type LEPKCompileOutput = {
    directoryFile: string;
    files: LEPKSegmentFileRaw[];
  };

  export type LEPKSegmentFileRaw = {
    filename: string;
    data: ArrayBuffer;
  };

  export function getParentUrl(path: string): URL {
  ...
  export function joinUrls(baseUrl: string, pathSegment: string): string {
  ...
  export function stringifyBlob(blob: Blob): Promise<string> {
  ...
  export async function compileLargeEPK(
    filename: string,
    file: ArrayBuffer,
  ...
  export class LargeEPK {
    url?: URL | string;
    rawData?: string | Blob;

    filename?: string;
    segments?: LargeEPKSegment[];
    hash?: string;
    partial: boolean = true;

    constructor(
      resource: URL | string | Blob,
      resourceType: PassedResourceType
    ) {
    ...
    public async fetchMetadata(): Promise<this> {
      ...
      const metadata: LEPKDirectoryFileRaw = JSON.parse(this.rawData!);
      ...
      if (
        ...
        for (const segment of metadata.segments) {
          if (typeof segment != "string") {
            throw new TypeError(
              `metadata.segments[${metadata.segments.indexOf(
                segment
              )}] must be a string!`
            );
          }
      ...
      this.segments = metadata.segments.map(
        (segment) => new LargeEPKSegment(isAbsoluteUrl(segment) ? segment : this.url != null ? segment.startsWith('/') ? new URL(segment, window.location.href) : joinUrls(getParentUrl(this.url instanceof URL ? this.url.toString() : this.url).toString(), segment) : segment)
      );
      return this;
    }

    public fetch(): { percent: number, progressCallback: EventTarget, promise: Promise<LargeEPK> } {
      if (this.segments === null) {
        throw new TypeError("Segments not initialized");
      }

      const eventTarget = new EventTarget();
      const promiseMeta: { res?: (epk: LargeEPK) => void, rej?: (err: Error) => void } = {}
      const ret: { percent: number, progressCallback: EventTarget, promise: Promise<LargeEPK> } = {
        percent: 0,
      ...
      this.segments!.forEach((segment) => {
        ...
        segmentEventTarget.addEventListener("progress", (event) => {
          ...
          (progressEvent as any).overallPercent = overallPercent;
      ...
      for (const segment of this.segments!) {
        if (segment.data != null) {
          if (!buffer) {
            ...
            const oldBuffer = buffer as ArrayBuffer,
              newBuffer = segment.data!;
            const tmp = new Uint8Array(
              oldBuffer.byteLength + newBuffer.byteLength
            );
          ...
          throw new Error(
            "One or more LargeEPKSegment(s) haven't been fetched yet. Did you fall fetch() beforehand?"
          );
        }
      }
      return buffer!;
    ...
    public disposeFetchedSegments(): this {
      if (this.segments == null) {
  ...
  export class LargeEPKSegment {
    ...
    public fetchSegment(): { promise?: Promise<LargeEPKSegment>, percent: number, eventTarget: EventTarget } {
      if (this.data != null) {
        throw new TypeError("Cannot call fetchSegment() twice!");
      }

      const eventTarget = new EventTarget();
      const ret: { promise?: Promise<LargeEPKSegment>, percent: number, eventTarget: EventTarget } = {
        promise: undefined,
        percent: 100, // Finished
        eventTarget,
      };
      const promise = new Promise<this>(async res => {
        ...
        while (true) {
          ...
          const progressEvent = new Event("progress");
          ;(progressEvent as any).percent = percent;
          ret.percent = percent;
